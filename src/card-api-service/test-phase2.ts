...
async function testPhase2Services() {
  ...
  try {
    ...
    const selectionStrategy =
      ProviderSelectionStrategyFactory.create('priority')
    ...
    console.log(
      '✅ ProviderSelectionStrategy fonctionne:',
      selectedProvider === 'scryfall'
    )

    // Test 6: FallbackService
    console.log('\n📋 Test 6: FallbackService')
    const { SimpleFallbackStrategy } = await import(
      './strategy/FallbackStrategy'
    )
    const _fallbackService = new FallbackService(
